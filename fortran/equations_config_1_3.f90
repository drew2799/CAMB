    ...
    real(dl) fid_fb, fb, frac_b, frac_c
    real(dl) mod_dgrho_matter, mod_dgrho, mod_dgq, mod_pb43, mod_z, mod_dz, mod_qg, mod_qr, mod_sigma
    real(dl) split_tau
    logical is_tau_post_drag
    ...
    ! Splitting equations at z~
    ! 488.08=tau(z=500) when drag epoch effects became negligible
    ! 280.53=tau_decoupling, 286.48=tau_dragepoch (Planck2018)
    split_tau = 280.53
    if (tau >= split_tau) then  
        is_tau_post_drag = .true.
    else
        is_tau_post_drag = .false.
    end if
    ...
    fid_fb=State%grhob/(State%grhob+State%grhoc)

    if (State%CP%gamma_b > 1._dl) then
        fb=State%grhob/(State%grhob+State%grhoc)
        frac_b=fb/fid_fb
    ...
    mod_dgrho_matter=frac_b*grhob_t*clxb+frac_c*grhoc_t*clxc

    !  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
    dgq=grhob_t*vb
    mod_dgq=frac_b*grhob_t*vb

    gpres_nu=0
    grhonu_t=0

    ! Not affecting 'cause I put Num_Nu_Massive = 0
    ...
    if (EV%no_nu_multpoles) then
        ...
        mod_z=(0.5_dl*mod_dgrho/k + etak)/adotoa
        dz= -adotoa*z - 0.5_dl*dgrho/k
        mod_dz= -adotoa*mod_z - 0.5_dl*mod_dgrho/k
        if (.not. is_tau_post_drag) then
            clxr=-4*dz/k
        else
            clxr=-4*mod_dz/k 
        end if
        qr=-4._dl/3*z
        mod_qr=-4._dl/3*mod_z
        pir=0
    else
        !  Massless neutrinos
        clxr=ay(EV%r_ix)
    ...
    if (EV%no_phot_multpoles) then
        if (.not. EV%no_nu_multpoles) then
            ...
            if (.not. is_tau_post_drag) then
                ...
                clxg=-4*mod_dz/k-4/k*opacity*(vb+mod_z)
            end if
            qg=-4._dl/3*z
            mod_qg=-4._dl/3*mod_z
        else
            if (.not. is_tau_post_drag) then
                clxg=clxr-4/k*opacity*(vb+z)
            else
                clxg=clxr-4/k*opacity*(vb+mod_z)
            end if
            qg=qr
            mod_qg=mod_qr
        end if
    ...
    mod_dgrho=mod_dgrho + grhog_t*clxg+grhor_t*clxr

    !  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
    dgq=dgq + grhog_t*qg+grhor_t*qr
    ! Pressure terms from photons and radiation as it should be with cosmological baryon fraction or growth baryon fraction?
    mod_dgq=mod_dgq + grhog_t*mod_qg+grhor_t*mod_qr

    !  Photon mass density over baryon mass density
    photbar=grhog_t/grhob_t
    pb43=4._dl/3*photbar
    mod_pb43=pb43/frac_b
    ...
    if (.not. EV%is_cosmological_constant) then
        ...
        mod_dgrho = mod_dgrho + dgrho_de
        dgq = dgq + dgq_de
        mod_dgq = mod_dgq + dgq_de
    end if
    ...
    if (State%flat) then
        ...
        mod_sigma=(mod_z+1.5_dl*mod_dgq/k2)
        if (.not. is_tau_post_drag) then
            ayprime(ix_etak)=0.5_dl*mod_dgq
        else
            ayprime(ix_etak)=0.5_dl*mod_dgq
        end if
    else
        sigma=(z+1.5_dl*dgq/k2)/EV%Kf(1)
        mod_sigma=(mod_z+1.5_dl*mod_dgq/k2)/EV%Kf(1)
        ayprime(ix_etak)=0.5_dl*mod_dgq + State%curv*mod_z
    end if
    ...
    if (.not. is_tau_post_drag) then
        ...
        clxgdot=-k*(4._dl/3._dl*mod_z+mod_qg)
    end if
    ...
    if (EV%Evolve_baryon_cs) then
        ...
        Delta_TM = clxg/4
        if (.not. is_tau_post_drag) then
            delta_p_b = cs2*clxb!*((1+pb43)/(1+mod_pb43))
        else
            delta_p_b = cs2*clxb!*frac_b
        end if
    end if
    ...
    if (EV%TightCoupling) then
    ...
    else if (EV%TightCoupling .and. is_tau_post_drag) then
        !  ddota/a
        gpres = gpres_noDE + w_dark_energy_t*grhov_t
        adotdota=(adotoa*adotoa-gpres)/2

        pig = 32._dl/45/opacity*k*(mod_sigma+vb)

        !  First-order approximation to baryon-photon splip
        slip = - (2*adotoa/(1+pb43) + dopacity/opacity)* (vb-3._dl/4*mod_qg) &
            +(-adotdota*vb-k/2*adotoa*clxg +k*(cs2*clxbdot-clxgdot/4))/(opacity*(1+pb43))
        ...
        if (second_order_tightcoupling) then
            ...
            ! Define shear derivative to first order
            sigmadot = -2*adotoa*mod_sigma-dgs/k+etak

            !Once know slip, recompute qgdot, pig, pigdot
            qgdot = k*(clxg/4._dl-pig/2._dl) +opacity*slip

            pig = 32._dl/45/opacity*k*(mod_sigma+3._dl*mod_qg/4._dl)*(1+(dopacity*11._dl/6._dl/opacity**2)) &
                + (32._dl/45._dl/opacity**2)*k*(sigmadot+3._dl*qgdot/4._dl)*(-11._dl/6._dl)

            pigdot = -(32._dl/45._dl)*(dopacity/opacity**2)*k*(mod_sigma+3._dl*mod_qg/4._dl)*(1 + &
                dopacity*11._dl/6._dl/opacity**2 ) &
                + (32._dl/45._dl/opacity)*k*(sigmadot+3._dl*qgdot/4._dl)*(1+(11._dl/6._dl) &
                *(dopacity/opacity**2))
    ...
    else if (.not. EV%TightCoupling .and. .not. is_tau_post_drag) then
        !vbdot=-adotoa*vb+k*delta_p_b*((1+pb43)/(1+mod_pb43))-(photbar/frac_b)*opacity*(4._dl/3*vb-mod_qg)
        vbdot=-adotoa*vb+k*delta_p_b-photbar*opacity*(4._dl/3*vb-qg)
    else if (.not. EV%TightCoupling .and. is_tau_post_drag) then
        vbdot=-adotoa*vb+k*delta_p_b-photbar*opacity*(4._dl/3*vb-mod_qg)
    end if
    ...
    if (.not. EV%no_phot_multpoles) then
        ...
        !Using mod_pb43 here is shifting the peak positions (changing derivative of photon pressure term is actually chaning BAO 'speed')
            qgdot=4._dl/3*(-vbdot-adotoa*vb+delta_p_b*k)/pb43 &
                +EV%denlk(1)*clxg-EV%denlk2(1)*pig
        end if
        ...
        if (.not. EV%tightcoupling) then
            if (.not. is_tau_post_drag) then
                ...
                ix= EV%g_ix+2
                if (EV%lmaxg>2) then
                    pigdot=EV%denlk(2)*mod_qg-EV%denlk2(2)*ay(ix+1)-opacity*(pig - polter) &
                        +8._dl/15._dl*k*mod_sigma
                    ayprime(ix)=pigdot
                    do  l=3,EV%lmaxg-1
                        ix=ix+1
                        ayprime(ix)=(EV%denlk(l)*ay(ix-1)-EV%denlk2(l)*ay(ix+1))-opacity*ay(ix)
                    ...
                    pigdot=EV%denlk(2)*mod_qg-opacity*(pig - polter) +8._dl/15._dl*k*mod_sigma
                    ayprime(ix)=pigdot
                endif
                !  Polarization
                !l=2
    ...
    if (.not. EV%no_nu_multpoles) then
        ...
        if (.not. is_tau_post_drag) then
            ...
            clxrdot=-k*(4._dl/3._dl*mod_z+mod_qr)
        end if
        ...
        if (EV%high_ktau_neutrino_approx) then
            ...
            if (EV%lmaxnr>2) then
                if (.not. is_tau_post_drag) then
                    ...
                    pirdot=EV%denlk(2)*mod_qr- EV%denlk2(2)*ay(ix+1)+8._dl/15._dl*k*mod_sigma
                end if
                ayprime(ix)=pirdot
                do l=3,EV%lmaxnr-1
                    ix=ix+1
                ...
                if (.not. is_tau_post_drag) then
                    ...
                    pirdot=EV%denlk(2)*mod_qr +8._dl/15._dl*k*mod_sigma
                end if
                ayprime(ix)=pirdot
            end if
        end if
    ...
    if (State%CP%Num_Nu_massive >0) then
        ...
        do nu_i = 1, State%CP%Nu_mass_eigenstates
            if (EV%MassiveNuApprox(nu_i)) then
                ...
                if (.not. is_tau_post_drag) then
                    ...
                    ayprime(off_ix)=-k*mod_z*(w+1) + 3*adotoa*(w*ay(off_ix) - ay(off_ix+1))-k*ay(off_ix+2)
                    ayprime(off_ix+1)=(3*w-2)*adotoa*ay(off_ix+1) - 5._dl/3*k*mod_z*w - k/3*G11_t
                    ayprime(off_ix+2)=(3*w-1)*adotoa*ay(off_ix+2) - k*(2._dl/3*EV%Kf(1)*ay(off_ix+3)-ay(off_ix+1))
                    ayprime(off_ix+3)=(3*w-2)*adotoa*ay(off_ix+3) + 2*w*k*mod_sigma - k/5*(3*EV%Kf(2)*G30_t-2*G11_t)
                end if
            else
                ind=EV%nu_ix(nu_i)
                ...
                do i=1,EV%nq(nu_i)
                    ...
                    if (.not. is_tau_post_drag) then
                        ...
                        ayprime(ind)=-k*(4._dl/3._dl*mod_z + v*ay(ind+1))
                    end if
                    ind=ind+1
        ...
        if (EV%has_nu_relativistic) then
            ...
            if (.not. is_tau_post_drag) then
                ...
                ayprime(ind)=+k*a2*mod_qr -k*ay(ind+1)
            end if
            ind2= EV%r_ix
    ...
    if (associated(EV%OutputTransfer) .or. associated(EV%OutputSources)) then
        ...
        if (EV%no_phot_multpoles) then
            ...
            if (EV%TightCoupling .and. .not. is_tau_post_drag) then
                if (second_order_tightcoupling) then
                ...
                if (second_order_tightcoupling) then
                    ...
                    pigdot = -dopacity/opacity*pig + 32._dl/45*k/opacity*(-2*adotoa*mod_sigma  &
                        +etak/EV%Kf(1)-  dgpi/k +vbdot )
                    Edot(2) = pigdot/4
                    E(2) = pig/4
                    octg=0
                ...
                octgdot=0
            else if (.not. EV%TightCoupling) then
                octg=ay(EV%g_ix+3)
        ...
        if (.not. is_tau_post_drag) then
            ...
            diff_rhopi = pidot_sum - (4*dgpi+ dgpi_diff)*adotoa + &
                State%CP%DarkEnergy%diff_rhopi_Add_Term(dgrho_de, dgq_de, grho, &
                gpres, w_dark_energy_t, State%grhok, adotoa, &
                EV%kf(1), k, grhov_t, mod_z, k2, ayprime, ay, EV%w_ix)
        end if
        if (.not. is_tau_post_drag) then
            phi = -((dgrho +3*dgq*adotoa/k)/EV%Kf(1) + dgpi)/(2*k2)
        else
            phi = -((mod_dgrho +3*mod_dgq*adotoa/k)/EV%Kf(1) + dgpi)/(2*k2)
        end if
        ...
        if (associated(EV%OutputTransfer)) then
            ...
            if (EV%TightCoupling) then
                ...
                EV%OutputTransfer(Transfer_Newt_vel_cdm)=  -k*mod_sigma/adotoa
                EV%OutputTransfer(Transfer_Newt_vel_baryon) = -k*(vb + mod_sigma)/adotoa
            end if
        ...
        if (associated(EV%OutputSources)) then
            ...
            if (.not. is_tau_post_drag) then
                ...
                phidot = (1.0d0/2.0d0)*(adotoa*(-dgpi - 2*k2*phi) + mod_dgq*k - &
                    diff_rhopi+ k*mod_sigma*(gpres + grho))/k2
                !time derivative of shear
                sigmadot = -adotoa*mod_sigma - 1.0d0/2.0d0*dgpi/k + k*phi
            end if
            ...
            if (.not. is_tau_post_drag) then
                ...
                polterddot = -2.0d0/25.0d0*adotoa*mod_dgq/(k*EV%Kf(1)) - 4.0d0/75.0d0*adotoa* &
                    k*mod_sigma - 4.0d0/75.0d0*dgpi - 2.0d0/75.0d0*mod_dgrho/EV%Kf(1) - 3.0d0/ &
                    50.0d0*k*octgdot*EV%Kf(2) + (1.0d0/25.0d0)*k*qgdot - 1.0d0/5.0d0 &
                    *k*EV%Kf(2)*Edot(3) + (-1.0d0/10.0d0*pig + (7.0d0/10.0d0)* &
                    polter - 3.0d0/5.0d0*E(2))*dopacity + (-1.0d0/10.0d0*pigdot &
                    + (7.0d0/10.0d0)*polterdot - 3.0d0/5.0d0*Edot(2))*opacity
            ...
            if (.not. is_tau_post_drag) then
                ...
                doppler = ((mod_sigma + vb)*dvisibility + (sigmadot + vbdot)*visibility)/k
            end if
            ...
            if (State%num_redshiftwindows > 0) then
                if (.not. is_tau_post_drag) then
                    ...
                    call output_window_sources(EV, EV%OutputSources, ay, ayprime, &
                        tau, a, adotoa, grho, gpres, &
                        k, etak, mod_z, ayprime(ix_etak), phi, phidot, mod_sigma, sigmadot, &
                        mod_dgrho, clxg,clxb,clxc,clxnu, Delta_TM, Delta_xe, &
                        mod_dgq, mod_qg, vb, qgdot, vbdot, &
                        dgpi, pig, pigdot, diff_rhopi, &
                        polter, polterdot, polterddot, octg, octgdot, E, Edot, &
                        opacity, dopacity, ddopacity, visibility, dvisibility, ddvisibility, exptau)
                end if
            ...
            if (associated(EV%CustomSources)) then
                ...
                block
                    ...
                    if (.not. is_tau_post_drag) then
                        ...
                        call custom_sources_func(EV%CustomSources, tau, a, adotoa, grho, gpres,w_dark_energy_t, cs2_de, &
                            frac_b*grhob_t,grhor_t,frac_c*grhoc_t,grhog_t,grhov_t,grhonu_t, &
                            k, etak, ayprime(ix_etak), phi, phidot, mod_sigma, sigmadot, &
                            mod_dgrho, clxg,clxb,clxc,clxr,clxnu, dgrho_de/grhov_t, delta_p_b, &
                            mod_dgq, mod_qg, mod_qr, dgq_de/grhov_t, vb, qgdot, qrdot, vbdot, &
                            dgpi, pig, pir, pigdot, pirdot, diff_rhopi, &
                            polter, polterdot, polterddot, octg, octgdot, E, Edot, &
                            opacity, dopacity, ddopacity, visibility, dvisibility, ddvisibility, exptau, &
                            tau0, State%tau_maxvis, EV%Kf,f_K)
